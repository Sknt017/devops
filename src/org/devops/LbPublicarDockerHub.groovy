...
class LbPublicarDockerHub{
    static void publicarImagen(steps){
        ...
        def imageExists = sh(script: "docker images -q ${dockerHubUsername}/termometro",returnStdout: true).trim()
        if(imageExists){
            echo 'La imagen ${dockerHubUsername}/termometro ya existe, descartando...'
        }else{
